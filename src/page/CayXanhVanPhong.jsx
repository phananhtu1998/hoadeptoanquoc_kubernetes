...
let sortedData = lstdatahoa.filter(item => item.Category === "cay-xanh-van-phong");

const CayXanhVanPhong = ({ setCartItems }) => {
    useEffect(() => {
        document.title = "Cây Xanh Văn Phòng";
        window.scrollTo({
            top: 760,
            left: 0,
        ...
        <div className={style.customstyles}>
            <Container>
                <h1 className={style.h1customtext}>
                    <span className={style.spantextcenter}>Cây xanh văn phòng</span>
                </h1>
            ...
            <Routes>
                {currentItems.map((index) => (
                    <Route
                        key={index}
                        path={`/cay-xanh-van-phong/${index + 1}`}
