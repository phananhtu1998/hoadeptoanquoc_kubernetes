...
let sortedData = lstdatahoa.filter(item => item.Category === "trai-cay-van-phong");

const TraiCayVanPhong = ({ setCartItems }) => {
    useEffect(() => {
        document.title = "Trái Cây Văn Phòng";
        window.scrollTo({
            top: 760,
            left: 0,
        ...
        <div className={style.customstyles}>
            <Container>
                <h1 className={style.h1customtext}>
                    <span className={style.spantextcenter}>Trái cây văn phòng</span>
                </h1>
            ...
            <Routes>
                {currentItems.map((index) => (
                    <Route
                        key={index}
                        path={`/trai-cay-van-phong/${index + 1}`}
...
export default TraiCayVanPhong;
