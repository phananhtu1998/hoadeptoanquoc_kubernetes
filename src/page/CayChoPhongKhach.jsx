...
let sortedData = lstDataHoa.filter(item => item.Category === "cay-cho-phong-khach");
const CayChoPhongKhach = ({ setCartItems }) => {
    useEffect(() => {
        document.title = "Cây Cho Phòng Khách";
        window.scrollTo({
            top: 760,
            left: 0,
        ...
        <div className={style.customstyles}>
            <Container>
                <h1 className={style.h1customtext}>
                    <span className={style.spantextcenter}>Cây cho phòng khách</span>
                </h1>
            ...
            <Routes>
                {currentItems.map((index) => (
                    <Route
                        key={index}
                        path={`/cay-cho-phong-khach/${index + 1}`}
...
export default CayChoPhongKhach;
