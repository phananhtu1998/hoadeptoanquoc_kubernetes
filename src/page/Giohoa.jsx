...
let sortedData = lstGioHoa.filter(item => item.Category === "gio-hoa" || item.Category === "hop-hoa");

const GioHoa = ({ setCartItems }) => {
    useEffect(() => {
        document.title = "Giỏ Hoa - Hộp Hoa";
        window.scrollTo({
            top: 760,
            left: 0,
        ...
        <div className={style.customstyles}>
            <Container>
                <h1 className={style.h1customtext}>
                    <span className={style.spantextcenter}>Giỏ hoa - Hộp hoa</span>
                </h1>
